...
def FitRoutine(FittingInfo, chunk_list):
    ...
    if save_failed_fits != False:
        if not os.path.exists('%s/failed_fits_1comp/' % savepath):
            os.makedirs('%s/failed_fits_1comp/' % savepath)
    if (save_failed_fits != False) and (nested_fit is True):
        if not os.path.exists('%s/failed_fits_2comp/' % savepath):
            os.makedirs('%s/failed_fits_2comp/' % savepath)
    if save_good_fits != False:
        if not os.path.exists('%s/good_fits_1comp/' % savepath):
            os.makedirs('%s/good_fits_1comp/' % savepath)
    if (save_good_fits != False) and (nested_fit is True):
        if not os.path.exists('%s/good_fits_2comp/' % savepath):
            os.makedirs('%s/good_fits_2comp/' % savepath)
    ...
    # this fit will save to a different cube.
        ...
        z, y, x = chunk.shape
        parcube1 = np.zeros((npars // 2,y,x))  # rows, columns
        parcube2 = np.zeros((npars,y,x))  # rows, columns
        RedChiSqCube1 = np.zeros((npars // 2,y,x))  # rows, columns
        RedChiSqCube2 = np.zeros((npars,y,x))  # rows, columns
        ...
        for i in np.arange(x): # x-axis  
            for j in np.arange(y): # y-axis
                ...
                params1 = [par for sublist in zip(amps1, cens1, sigmas1)
                           for par in sublist]
                 
                parcube1[:,j,i] = params1
                RedChiSqCube1[:,j,i] = redchisq1
                ...
                if save_failed_fits != False:
                    if (redchisq1 > upperlim) and (count % save_failed_fits == 0):
                        
                         # print the fit
                         plot_one_fit(i, j, spec1, redchisq1, 
                                     savepath = '%s/failed_fits_1comp/' % savepath, 
                                     xmin=6530, xmax=6620, 
                                     ymax=max(spectrum), fluxnorm=1e-20,
                                     input_params = total_guesses)
                     
                # do we want to save good fits?
                # save_good_fits is an integer that tells you to 
                # save every n good fits
                if save_good_fits != False:
                    if (redchisq1 <= upperlim) and (count % save_good_fits == 0):
                         
                         # print the fit
                         plot_one_fit(i, j, spec1, redchisq1, 
                                       savepath = '%s/good_fits_1comp/' % savepath, 
                                       xmin=6530, xmax=6620, 
                                       ymax=max(spectrum), fluxnorm=1e-20,
                                       input_params = total_guesses)
                ...
                # do we want to re-do the fit but now with
                # more parameters?
                if nested_fit == True:
                    ...
                    # save everything to the parameter cube!!!!
                    params2 = [par for sublist in zip(amps2, cens2, sigmas2)
                               for par in sublist]
                    ordered_params2 = component_order_check(params2)
                    parcube2[:,j,i] = ordered_params2
                    RedChiSqCube2[:,j,i] = redchisq2
                    ...
                    # flagging will involve printing out the fit and pixel coord.
                    if save_failed_fits != False:
                        if (redchisq2 > upperlim) and (count % save_failed_fits == 0):
                            
                            # print the fit
                            plot_one_fit(i, j, spec2, redchisq2, 
                                          savepath = '%s/failed_fits_2comp/' % savepath, 
                                          xmin=6530, xmax=6620, 
                                          ymax=max(spectrum), fluxnorm=1e-20,
                    ...
                    # save every n good fits
                    if save_good_fits != False:
                        if (redchisq2 <= upperlim) and (count % save_good_fits == 0):
                            
                            # print the fit
                            plot_one_fit(i, j, spec2, redchisq2, 
                                          savepath = '%s/good_fits_2comp/' % savepath, 
                                          xmin=6530, xmax=6620, 
                                          ymax=max(spectrum), fluxnorm=1e-20,
                                          input_params = nested_total_guesses)
        ...
        parname = ['Amplitude', 'Center', 'Width'] * int(npars // 2 // 3)
        jj = 0
        ...
        try:
            hdul1.writeto('%s/fit_%s_1comp.fits' % (savepath, chunk_num), overwrite=True)
        except:
            hdul1.writeto('fit_%s_1comp.fits' % chunk_num, overwrite=True)
            
        hdr1_chi = fits.Header()
        hdr1_chi['NOTE'] = 'reduced chi square - 1 component'
        
        hdul1_chi = fits.PrimaryHDU(data=RedChiSqCube1, header=hdr1_chi)
            
        try:
            hdul1_chi.writeto('%s/redchisq_%s_1comp.fits' % (savepath, chunk_num), overwrite=True)
        except:
            hdul1_chi.writeto('redchisq_%s_1comp.fits' % chunk_num, overwrite=True)
        ...
        try:
            hdul2.writeto('%s/fit_%s_2comp.fits' % (savepath, chunk_num), overwrite=True)
        except:
            hdul2.writeto('fit_%s_2comp.fits' % chunk_num, overwrite=True)

        hdr2_chi = fits.Header()
        hdr2_chi['NOTE'] = 'reduced chi square - 1 component'
        
        hdul2_chi = fits.PrimaryHDU(data=RedChiSqCube2, header=hdr2_chi)
            
        try:
            hdul2_chi.writeto('%s/redchisq_%s_2comp.fits' % (savepath, chunk_num), overwrite=True)
        except:
            hdul2_chi.writeto('redchisq_%s_2comp.fits' % chunk_num, overwrite=True)
    return
