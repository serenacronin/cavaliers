...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 12 13:52:26 2022

@author: serenac

NEW VERSION OF CUBEFITROUTINE.py BUT WE ARE GOING TO ADD CRITERIA ABOUT
RESIDUALS. THIS IS ADDING ON TO CUBEFITROUTINEV2.py.
"""
        ...
        for i in np.arange(y): # y-axis     
            ...
            for j in np.arange(x): # x-axis
                ...
                redchisq1 = red_chisq(spectrum, model1,
                                      num_params=len(amps1)*3, err=errs1)
                ...
                if nested_fit_threshold is not False:
                    ...
                    redchisq2 = red_chisq(spectrum, model2,
                                          num_params=len(amps2)*3, err=errs2)
                    
                    
                    
                    # # see if the reduced chi-square is less than 1
                    # # the data seems to like that
                    # # therefore, use that fit
                    # if (redchisq1 < 1.0) & (redchisq2 > 1.0):
                    #     redchisq = redchisq1
                    #     spec = spec1
                    #     amps = amps1
                    #     cens = cens1
                    #     sigmas = sigmas1
                    # elif (redchisq2 < 1.0) & (redchisq1 > 1.0):
                    #     redchisq = redchisq2
                    #     spec = spec2
                    #     amps = amps2
                    #     cens = cens2
                    ...
                    # else: # okay, none are less than 1. what now?
                        # see which fit has a redchisq closest to 1!
                        
                    if np.abs(1 - redchisq1) > np.abs(1 - redchisq2):
                        redchisq = redchisq2
                        amps = amps2
                        cens = cens2
                        sigmas = sigmas2
                    ...
                    # residual tolerance; right now, arbitrarily allowing for less than 2 channels to be above 80
                    elif len(np.where(np.abs(spec1.specfit.residuals) > 80.)[0]) >= 2:
                        redchisq = redchisq2
                        amps = amps2
                        cens = cens2
                        sigmas = sigmas2
                        ...
                        # amps = amps1 + [np.nan,np.nan,np.nan]
                        # cens = cens1 + [np.nan,np.nan,np.nan]
                        # sigmas = sigmas1 + [np.nan,np.nan,np.nan]
